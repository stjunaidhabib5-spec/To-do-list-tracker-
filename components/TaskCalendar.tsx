      ...
      >
        ...
        <div className="mt-1 flex flex-col gap-0.5 w-full text-left">
          {dayTasks.slice(0, 3).map((task, idx) => {
            const isDone = Boolean(task?.is_completed ?? (task as any)?.completed);
            const title = task?.title || 'Untitled Task';
            const taskId = task?.id || `task-fallback-${idx}`;
            return (
            <div
              key={taskId}
              onClick={(e) => {
            ...
            return (
              ...
              >
                ...
                <div className="mt-4 flex flex-col gap-2.5 max-h-[60vh] overflow-y-auto pr-1 no-scrollbar">
                  {selectedDayTasks.tasks.map((task, idx) => {
                    const isDone = Boolean(task?.is_completed ?? (task as any)?.completed);
                    const title = task?.title || 'Untitled Task';
                    const taskId = task?.id || `modal-task-${idx}`;
                    return (
                    <div 
                      key={taskId}
                      onClick={() => {
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <input type="checkbox" checked={isDone} readOnly className="rounded-full text-emerald-600 focus:ring-0" />
                        <span className={`font-handwriting text-xl font-semibold text-slate-900 ${isDone ? 'line-through opacity-70 text-slate-600' : ''}`}>{title}</span>
                      </div>
                      <span className="font-handwriting text-xs font-bold px-2.5 py-0.5 rounded-md bg-black/10 text-slate-800 tracking-wider">{task?.category || 'General'}</span>
                    </div>
                    );
                  })}
                </div>
      ...
      {selectedTaskDetail && (
        ...
        >
          {(() => {
            ...
            const isDone = Boolean(selectedTaskDetail?.is_completed ?? (selectedTaskDetail as any)?.completed);
            const title = selectedTaskDetail?.title || 'Untitled Task';
            const category = selectedTaskDetail?.category || 'General';
            ...
            return (
              ...
              >
                ...
                <div className="mb-4 pr-8">
                  <h3 className={`font-handwriting text-4xl font-bold leading-tight ${isDone ? 'line-through opacity-70 text-slate-600' : 'text-slate-900'}`}>
                    {title}
                  </h3>
                </div>
